...
# Konversi representasi biner ke ekspresi Boolean
def binary_to_expression(binary, variables):
    expression = []
    for i, bit in enumerate(binary):
        if bit == '1':
            expression.append(variables[i])  # Variabel asli
        elif bit == '0':
            expression.append(f"{variables[i]}'")  # Negasi variabel
...
if __name__ == "__main__":
    try:
        ...
        # Buat daftar variabel (w, x, y, z, ...)
        variables = [chr(i) for i in range(119, 119 + num_vars)]
        
        # Outputkan hasil
        print("\nEssential Prime Implicants:")
        if result:
            simplified_expression = " + ".join(binary_to_expression(r, variables) for r in result)
            for r in result:
                print(f" - {r}")  # Cetak setiap implicant
            print(f"Ekspresi Boolean : f({','.join(variables)}) = {simplified_expression}")
        else:
